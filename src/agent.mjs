...
import { sanitizarLinks } from './link_guard.mjs';
...
import * as MANUAL from './manual_proprietario.mjs';
...
import * as CLUBE from './clube.mjs';
import * as COBRADOR from './cobrador.mjs';
...
const TOOLS = [
  { type: 'function', function: { name: 'resolver_cadastro', description: 'Identifica a(s) unidade(s) da pessoa. Prefira por CPF; se ela não tem/não sabe o CPF, busque por NOME + UNIDADE (bloco e apartamento) + condomínio — nome + unidade dá confiança ALTA (o apartamento restringe e o nome confirma). Retorna { encontrado, criterio (cpf|unidade_nome|telefone|nome_exato|nome_completo|nome_parcial), confianca (alta|media|baixa), unidades:[{id_unidade, identificacao (bloco/unidade), condominio, id_condominio, papel, nome, ex_morador}] }. confianca ALTA = CPF/telefone/unidade+nome (é a própria pessoa) → pode prosseguir. confianca MEDIA/BAIXA = achou só por NOME (homônimo possível) ou só pela unidade → CONFIRME um 2º dado (a unidade/bloco, ou parte do CPF) ANTES de entregar boleto/valor/dado sensível. motivo nome_exige_condominio = peça o condomínio. motivo condominio_nao_localizado = o nome do condomínio que a pessoa escreveu (vem em condominio_informado) NÃO existe na nossa carteira, provavelmente por erro de digitação: diga que não localizou ESSE nome, repita como ela escreveu e peça o nome completo ou o endereço. NUNCA ofereça unidades, blocos, garantidora ou boleto nesse caso — o candidato viria de outro prédio, e "403" existe em dezenas deles. Use antes de qualquer ação que dependa da unidade.', parameters: { type: 'object', properties: { cpf: { type: 'string', description: 'CPF da pessoa (com ou sem máscara).' }, nome: { type: 'string', description: 'Nome completo (use quando a pessoa não tem/sabe o CPF).' }, condominio: { type: 'string', description: 'Nome do condomínio. Obrigatório na busca por nome/unidade.' }, unidade: { type: 'string', description: 'Unidade da pessoa (bloco/torre e apartamento, ex.: "Bloco 7 ap 401", "apto 142 torre 2"). Combine com nome quando não há CPF.' } } } } },
  { type: 'function', function: { name: 'get_boleto_2via', description: '2ª via do boleto pendente de uma unidade: retorna PIX copia-e-cola (st_pixqrcode_recb) e link. Vencido +30 dias retorna liberado:false motivo:"boleto vencido +30 dias" (encaminhar à cobrança). Unidade em PROCESSO JUDICIAL retorna liberado:false motivo:unidade_no_juridico → NÃO mande PIX nem link (o boleto público fica indisponível "porque a unidade está no jurídico"); encaminhe à cobrança. Se vier `pix_disponivel:false` com `nota_pix`, o boleto desse condomínio é emitido por OUTRO banco e NÃO tem PIX — repasse o texto de `nota_pix` como veio e NUNCA diga "não consegui obter o PIX agora"/"no momento" (não é falha temporária, é permanente: sugerir que pode dar certo depois faz a pessoa tentar de novo à toa). Siga entregando link e PDF normalmente. Se vier `outras_cobrancas_em_aberto` (N>0) junto com `aviso_morador`, entregue o boleto normalmente E acrescente o texto de `aviso_morador` — a 2ª via só enxerga os ~30 dias recentes, então há débito mais antigo que ela NÃO mostra; repasse esse texto como veio (não recalcule o número nem cite valores) e ofereça pedir a relação completa à cobrança. MÊS ESPECÍFICO: quando a pessoa pedir o boleto DE UM MÊS ("quero o de julho", "o boleto de 07/2026", "o do mês passado"), passe `mes` no formato AAAA-MM — sem isso você entrega o do mês CORRENTE e a pessoa recebe o boleto errado. Nesse caso o retorno traz `situacao`: `pago` = o boleto JÁ FOI PAGO (em `dt_liquidacao_recb`) — diga isso com todas as letras e entregue o link só como DOCUMENTO (comprovante/sinistro/imposto de renda); NUNCA apresente como "2ª via para pagamento" e não haverá PIX, de propósito. `em_aberto` = 2ª via normal. Motivos possíveis: `sem_boleto_no_mes` (não há boleto com vencimento nesse mês para a unidade — diga isso, não invente outro mês) e `mes_invalido`. A régua é a data de VENCIMENTO. Se vier `nota_extra` junto com motivo:garantidora, repasse esse texto como veio DEPOIS de indicar a garantidora — nesse condomínio existe uma taxa extra que você não enxerga e que a equipe consegue; sem isso a pessoa ouve "não é conosco" sobre algo que é. Exige id_condominio e id_unidade (do resolver_cadastro).', parameters: { type: 'object', properties: { id_condominio: { type: 'string' }, id_unidade: { type: 'string' }, mes: { type: 'string', description: 'Opcional. Mês do VENCIMENTO no formato AAAA-MM (ex.: "2026-07"). Só preencha quando a pessoa pedir um mês específico; sem isso vem o boleto em aberto do momento.' } }, required: ['id_condominio', 'id_unidade'] } } },
  { type: 'function', function: { name: 'get_inadimplencia', description: 'Situação COMPLETA de débitos da unidade — inclui boletos ANTIGOS, em cobrança e jurídico (não só os recentes). USE em "estou devendo?", "quanto devo?", "só devo esse boleto?", "estou quitado?". Retorna { status }: `inadimplente` (+`qtd_cobrancas_em_aberto`) = há débitos em aberto; `sem_debito_vencido` = não consta inadimplência (mas pode haver boleto A VENCER → get_boleto_2via); `gerido_por_garantidora` = cobrança pela garantidora; `indisponivel` = consulta falhou. `no_juridico:true` = PROCESSO JUDICIAL aberto → NÃO ofereça 2ª via/PIX, encaminhe à cobrança. NUNCA afirme um valor total a pagar. QUAIS MESES: se a pessoa quiser a RELAÇÃO das cobranças ("quais meses eu devo?", "me manda a relação", "quais estão em aberto?"), chame com `detalhar:true` — volta `detalhe` com `cobrancas[]` (vencimento, valor, dias_vencido, em_acordo) e `total_original`. Liste os itens como vieram. `total_original` é a soma dos valores ORIGINAIS: apresente sempre junto do texto de `detalhe.nota_valor` e NUNCA como "total a pagar"/"você deve X" — falta juros, multa e honorários, que a cobrança calcula. Se vier `truncado:true`, diga que são as mais antigas e que a relação completa vem com a equipe.', parameters: { type: 'object', properties: { id_condominio: { type: 'string' }, id_unidade: { type: 'string' }, detalhar: { type: 'boolean', description: 'Opcional. true quando a pessoa quer saber QUAIS cobranças estão em aberto, não só se há débito.' } }, required: ['id_condominio', 'id_unidade'] } } },
  { type: 'function', function: { name: 'enviar_anexo_pdf', description: 'Envia o PDF da 2ª via do boleto como ANEXO no chat da pessoa. Chame SEMPRE logo após get_boleto_2via com liberado:true (mesmos id_condominio e id_unidade), no MESMO turno — o boleto em PDF vai sempre junto, sem esperar a pessoa pedir. Retorna { enviado, vencimento, valor } ou { enviado:false, motivo } (ex.: sem boleto pendente, vencido +30 dias, garantidora). Não descreva o conteúdo do PDF nem invente valores; o texto da sua resposta (PIX + valor + vencimento + link) é entregue na mesma mensagem do anexo. O PIX copia-e-cola é o jeito mais rápido de pagar; o PDF é o complemento. Se você chamou get_boleto_2via com `mes`, passe O MESMO `mes` aqui — senão o anexo vem do mês corrente e a legenda diz outro mês.', parameters: { type: 'object', properties: { id_condominio: { type: 'string' }, id_unidade: { type: 'string' }, mes: { type: 'string', description: 'Opcional, AAAA-MM. Use o mesmo valor passado ao get_boleto_2via.' } }, required: ['id_condominio', 'id_unidade'] } } },
  ...
  { type: 'function', function: { name: 'enviar_manual_proprietario', description: 'Envia ao morador o MANUAL DO PROPRIETARIO do imovel, em PDF e INTEIRO, como anexo. USE quando a duvida for sobre o APARTAMENTO em si: garantia, prazo de garantia, manutencao preventiva, acabamento, piso, esquadria, pintura, louca, metais, impermeabilizacao, o que faz perder a garantia, quem chamar para assistencia tecnica. NAO use para regra de convivencia, area comum, mudanca, animais, barulho, obra, taxa ou documento do condominio — isso e consultar_regimento / consultar_regra_mudanca / consultar_base_geral. So alguns condominios tem manual cadastrado; ok:false com motivo condominio_sem_manual significa que aquele predio NAO tem, e ai voce diz que nao tem o manual dele e oferece a equipe — nunca mande o manual de outro condominio. Retorna { enviado, titulo, condominio, paginas } ou { enviado:false, motivo }. O documento vai completo de proposito (decisao do Fernando): nao resuma, nao cite trecho e nao descreva o conteudo — apenas avise que esta enviando o manual.', parameters: { type: 'object', properties: { condominio: { type: 'string', description: 'Nome do condominio da pessoa (use o do resolver_cadastro; se nao souber, pergunte).' } }, required: ['condominio'] } } },
  ...
  { type: 'function', function: { name: 'consultar_carteira_ncs', description: 'Confirma se um condomínio é administrado pelo Grupo NCS. USE sempre que perguntarem "vocês administram o X?", "o X é de vocês?", "quem administra o X?" — inclusive de quem está comprando/alugando apartamento, imobiliária ou outra administradora. Retorna { encontrou, nome, ambiguo, candidatos, resumo }. encontrou=true → confirme pelo nome. ambiguo=true → pergunte qual dos candidatos. encontrou=false → NUNCA diga que a NCS não administra (pode ser outro nome ou cliente novo): peça o nome completo ou o endereço e ofereça confirmar com a equipe.', parameters: { type: 'object', properties: { condominio: { type: 'string', description: 'Nome do condomínio como a pessoa escreveu.' } }, required: ['condominio'] } } },
  ...
  { type: 'function', function: { name: 'verificar_cobranca_oficial', description: 'Confere se uma cobranca que a pessoa RECEBEU e oficial. USE sempre que alguem perguntar se uma cobranca/mensagem/ligacao e golpe, se e verdadeira, se realmente e do condominio, ou disser que recebeu cobranca de um escritorio/telefone/e-mail que nao reconhece. Passe `condominio` e `quem` (o nome, telefone ou e-mail que apareceu na mensagem, como a pessoa escreveu). Retorna `status`: `confere` = e oficial, repasse o texto de `mensagem` como veio (explica que o escritorio e contratado PELO CONDOMINIO, nao pela NCS); `nao_confere` = NAO diga que e golpe, NUNCA use as palavras golpe/fraude/falso - repasse `mensagem` e transfira para a equipe; `sem_quem` = pergunte o nome que apareceu na mensagem; `condominio_ambiguo` = pergunte qual dos `candidatos`; `condominio_desconhecido`/`base_indisponivel` = transfira. Quando vier `transferir_humano:true`, chame transferir_humano no mesmo turno. Esta consulta NAO trata de juros, multa ou negociacao - se a pessoa quiser negociar valores, encaminhe a equipe de cobranca.', parameters: { type: 'object', properties: { condominio: { type: 'string', description: 'Nome do condominio da unidade.' }, quem: { type: 'string', description: 'Nome, telefone ou e-mail de quem cobrou, como a pessoa informou. Deixe vazio se ela ainda nao disse.' } }, required: ['condominio'] } } },
  { type: 'function', function: { name: 'consultar_clube', description: 'Consulta as empresas parceiras e os descontos do Clube NCS de Vantagens. Lista GLOBAL, igual para TODOS os condomínios — NÃO precisa (nem deve) do condomínio da pessoa. USE em "quais empresas dão desconto", "tem desconto em alguma farmácia/restaurante/pet", "o Clube NCS tem X". Passe um termo opcional (nome da empresa ou categoria, ex.: "GIOlaser", "Alimentação"); sem termo, devolve a lista completa (nome + categoria) para a pessoa escolher. Retorna { encontrou, empresas:[{nome, categoria, condicao, endereco, contato}] } ou { encontrou, lista:[{nome, categoria}] } quando sem termo. Repasse a condição (desconto) e o contato/endereço EXATAMENTE como vieram, sem resumir nem arredondar o percentual. encontrou=false (empresa_nao_encontrada/base_clube_vazia) → NUNCA invente empresa nem desconto; diga que não achou essa parceria.', parameters: { type: 'object', properties: { termo: { type: 'string', description: 'Nome da empresa ou categoria de interesse (ex.: "farmácia", "Drogaven"). Opcional — sem termo, lista tudo.' } } } } },
  { type: 'function', function: { name: 'enviar_cnd', description: 'Gera e envia a DECLARAÇÃO DE QUITAÇÃO DE DÉBITOS (CND) — via INFORMATIVA, SEM assinatura — como PDF no chat. Use quando a pessoa pedir "nada consta", "declaração de quitação", "CND", "comprovante de que estou em dia/quitado". Exige id_condominio e id_unidade (do resolver_cadastro). SÓ gera para quem está 100% em dia: se houver débito retorna { enviado:false, motivo:"inadimplente" } (direcione à Negociação de Débitos), processo judicial { motivo:"no_juridico" } (jurídico/cobrança), garantidora { motivo:"garantidora_ou_cego" } (canal da garantidora), financiamento externo { motivo:"financiamento_externo", detalhe } (o condomínio tem financiamento fora do Superlógica — ex.: reforma via 6P Bank no Vancouver; NÃO afirme que está quitado, explique conforme o "detalhe" e encaminhe para a equipe confirmar o saldo) ou { motivo:"indisponivel" } (ofereça atendente) — NUNCA afirme quitação quando não gerar. Esta é a via informativa (de conferência); a via OFICIAL assinada pelo síndico é solicitada à parte. Ao enviar, só confirme que mandou o PDF — não invente/transcreva o conteúdo.', parameters: { type: 'object', properties: { id_condominio: { type: 'string' }, id_unidade: { type: 'string' } }, required: ['id_condominio', 'id_unidade'] } } },
...
async function runToolReal(name, args, ctx) {
  switch (name) {
    ...
    case 'enviar_cnd': {
      ...
      if (!r.ok) return { enviado: false, motivo: r.motivo, ...(r.detalhe ? { detalhe: r.detalhe } : {}), ...(r.qtd_cobrancas_em_aberto != null ? { qtd_cobrancas_em_aberto: r.qtd_cobrancas_em_aberto } : {}) };
      if (ctx.chatId) {
    ...
    case 'enviar_manual_proprietario': {
      const r = MANUAL.consultar_manual_proprietario(args);
      if (!r.ok) return { enviado: false, motivo: r.motivo, ...(r.candidatos ? { candidatos: r.candidatos } : {}) };
      // Octadesk (WhatsApp) envia direto; nos demais canais o adapter baixa a URL e posta o anexo.
      if (ctx.chatId) {
        try {
          await OCTA.enviar_anexo_url({ chatId: ctx.chatId, sourceUrl: r.url, filename: r.filename, body: 'Segue o Manual do Proprietario 📄' });
          return { enviado: true, titulo: r.titulo, condominio: r.condominio, paginas: r.paginas };
        } catch (e) { return { enviado: false, motivo: 'falha_envio', detalhe: e.message }; }
      }
      (ctx.attachments ||= []).push({ url: r.url, filename: r.filename, kind: 'pdf' });
      return { enviado: true, canal_externo: true, titulo: r.titulo, condominio: r.condominio, paginas: r.paginas };
    }
    ...
    case 'consultar_carteira_ncs': return CARTEIRA.consultar_carteira_ncs(args);
    ...
    case 'verificar_cobranca_oficial': return COBRADOR.verificar_cobranca_oficial(args);
...
export async function runAgentLoop(session, systemPrompt, userText, ctx, runTool) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    // REGRA Nº 1 — cinto de segurança determinístico. O prompt já proíbe compor URL e o gate tem o
    // detector HALLUC_LINK, mas em 27/07 escapou 1 em ~50: a Ana mandou `gruponcs.net/ticket-mudanca`
    // (não existe; o certo é autorizacao-de-mudanca) para uma moradora, conv 257. Link fora da
    // allowlist da base sai da resposta E do histórico — senão ela repete o link errado nos turnos
    // seguintes por já estar no contexto.
    const lg = sanitizarLinks(reply);
    if (lg.removidos.length) {
      reply = lg.texto;
      console.warn('[link_guard] link inventado removido:', lg.removidos.join(','));
    }
    session.messages.push({ role: 'assistant', content: reply });
