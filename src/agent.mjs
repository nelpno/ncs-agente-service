...
const TOOLS = [
  ...
  { type: 'function', function: { name: 'get_inadimplencia', description: 'Situação COMPLETA de débitos da unidade — inclui boletos ANTIGOS, em cobrança e jurídico (NÃO só os recentes). USE quando a pessoa perguntar "estou devendo?", "quanto devo?", "só devo esse boleto?", "estou quitado?". Retorna { status }: `inadimplente` (+`qtd_cobrancas_em_aberto`) = HÁ débitos em aberto (pode ser muito além de um boleto recente) → diga que constam débitos em aberto (pode mencionar a quantidade) e direcione ao formulário de Negociação de Débitos ou a um atendente; NUNCA afirme um valor total a pagar. `sem_debito_vencido` = não consta inadimplência (mas pode haver boleto A VENCER → get_boleto_2via); não crave "quitado" como certeza. `gerido_por_garantidora` = cobrança pela garantidora. `indisponivel` = consulta falhou → ofereça atendente/CND, não afirme nada. Quando vier `no_juridico:true` (a unidade tem PROCESSO JUDICIAL aberto), NÃO ofereça 2ª via/PIX: explique com cuidado que a cobrança está em fase jurídica e encaminhe à cobrança (transferir_humano motivo cobranca).', parameters: { type: 'object', properties: { id_condominio: { type: 'string' }, id_unidade: { type: 'string' } }, required: ['id_condominio', 'id_unidade'] } } },
  ...
  { type: 'function', function: { name: 'consultar_regra_mudanca', description: 'Consulta a REGRA DE MUDANÇA do condomínio da pessoa, com conteúdo SEGURO PARA O MORADOR. USE SEMPRE que a pessoa for agendar ou perguntar sobre mudança. Passe o condomínio identificado no resolver_cadastro. Retorna { encontrou, condominio, horario (faixa de horário permitida), regras_condominio (antecedência específica e "1 por dia", quando houver), regras_gerais (mudança sem taxa; antecedência recomendada de 72h; agendar pelo formulário 24h ou atendente 8h–17h45; aguardar o termo de autorização; e que QUEM comunica a portaria/zeladoria e cadastra nos sistemas é a própria NCS) }. ⚠️ NUNCA mande o morador avisar/contatar a portaria, a zeladoria ou o síndico, nem cadastrar em sistema (Shielder etc.) — isso é PROCEDIMENTO INTERNO da administração e a ferramenta nem te entrega esse texto; o morador só preenche o formulário e aguarda. Se encontrou=false (condominio_nao_informado ou condominio_sem_regra), peça o condomínio ou ofereça confirmar o horário com a equipe — NUNCA invente horário. Para regras de CONVIVÊNCIA (animais, barulho, obras, áreas comuns) use consultar_regimento.', parameters: { type: 'object', properties: { condominio: { type: 'string', description: 'Nome ou slug do condomínio da pessoa (ex.: Lume). Use o condomínio identificado no resolver_cadastro; se ainda não souber, pergunte.' } }, required: ['condominio'] } } },
