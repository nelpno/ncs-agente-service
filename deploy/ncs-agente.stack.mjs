...
export const ANA_IMAGE = "ghcr.io/nelpno/ncs-agente-service@sha256:a1651820c2be640574c6e2d733f5f4755990e9ed1b7fd2663c2dba4cd446a44a"; // 9660047: identificacao por unidade+nome deixa de exigir a grafia do ERP. (1) ZERO A ESQUERDA: `_match` comparava o numero da unidade como TEXTO ("051" === "51" e falso), entao quem dissesse "apartamento 51" nao era achado - 2.325 das 3.798 unidades (61%), em 48 dos 59 condominios, sao gravadas com zero a esquerda; no Rosa de Ouro sao 100%. O match normalizado vale 1 ponto MENOS que o exato, entao resolver_cadastro (que so devolve os matches de score maximo) descarta a normalizada quando existe a exata - e o que mantem "10 G" e "010 G" (DONOS DIFERENTES no Tivoli) separadas. (2) NOME DO CONDOMINIO: o filtro era includes() cru, sem acento/plural/palavra-do-meio; 12 dos 55 nomes das NOSSAS bases nao casavam - entre eles "ROSAS DE OURO" (o ERP grava "ROSA DE OURO"), e sem match a busca varria os 59 condominios trazendo a unidade "051" de outros predios junto. Virou `_filtrarCondos` (pura, exportada): escada de 4 degraus (substring sem acento em FRONTEIRA DE PALAVRA, palavras significativas, singular, prefixo), reusando normNome/tokensNome/casaPorTokens do gerador. A fronteira de palavra existe porque tirar o acento fez "SALTO GRANDE I" casar dentro de "SALTO GRANDE III" (o pino de nao-regressao pegou). Ambiguidade legitima segue ambigua ("Cedros" devolve 2). Provas: test_unidade_zero_esquerda (15) + test_condominio_grafia (20) escritos antes do fix, suite 120/0, pino sobre os 59 nomes reais 47 iguais/8 melhoraram/0 degradaram/0 colisoes. DRY_RUN mantido; Frente C DORMANTE. Anterior 2c3172d: boleto SEM PIX explica o motivo em vez de sugerir falha temporaria. O Fernando confirmou (27/07) que o boleto do Parque Atlanta e emitido pelo SICOOB, nao pelo Superlogica -> 0 de 174 com PIX, e permanente (nao e configuracao faltando). A Ana dizia "No momento, nao consegui obter o PIX", o que faz o morador tentar de novo a toa; agora vem `pix_disponivel:false`+`nota_pix` (const NOTA_PIX_INDISPONIVEL, exportada p/ o teste conferir o texto REAL). Traz junto 50009d0: achados do 1o dia de volume real (27/07, 64 conversas). (1) get_boleto_2via cruza com a inadimplencia e devolve `outras_cobrancas_em_aberto`+`aviso_morador` - a regua de 2a via NAO enxerga o que venceu ha +30d (medido: nem com status=todos passa de 21 dias), pedido do Fernando na conv 186. (2) src/link_guard.mjs: link gruponcs.net fora da allowlist da base sai da resposta E do historico (a Ana mandou /ticket-mudanca, inexistente, na conv 257). (3) prompt: proibida a frase de escape, usar dado recem-informado no mesmo turno, nome citado = destinatario. DRY_RUN mantido; Frente C DORMANTE
