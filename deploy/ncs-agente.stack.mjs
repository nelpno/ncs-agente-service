...
export const ANA_IMAGE = "ghcr.io/nelpno/ncs-agente-service@sha256:27a124f8163ade71643188bfd8de88102050755a73227f5a75cef8a66f754cc8"; // f5f7891: as duas correcoes que o stress de 08/08 pediu antes de ligar WRITE_REAL_ACTIONS. (1) O norm() do cadastro_inquilino nao colapsava espaco INTERNO e o ERP guarda "DANIEL  PAGANIN" com dois espacos - ninguem digita assim, entao o checarConflito falhava CALADO e a duplicata passava. Provado no container com controle positivo (nome digitado=false, nome byte a byte do ERP=true); 10 de 1.187 contatos numa amostra de 5 condominios tem isso. Com a gravacao ligada seria a mesma pessoa cadastrada 2x na unidade. Colapsar NAO abre para nome parcial, que tem teste de controle. (2) Uma conversa em 48 preparou o MESMO cadastro duas vezes (S12) - e na fila real de 07/08 a Maria Poliana tinha 2 rascunhos do mesmo cadastro. Novo src/write/dedupe_rascunho.mjs + ENGINE.substituirRascunho: SUBSTITUI (o 2o card pode ser a correcao de um dado), so toca rascunho pendente, e FECHA A LINHA DA FILA junto. Kill-switch DEDUPE_RASCUNHO=0. Os cenarios feitos de proposito para cada defeito PASSARAM - os dois apareceram onde eu nao estava olhando. Suite 109/0; medicao em proposta/stress-cadastro-08ago.md. DRY_RUN_WRITES=true MANTIDO. Leva junto 4eeec55/4fc56a8/2c548d5 (atas saem da Ana e passam a viver so no Estagiario - fechava um vazamento de PII vivo: 25 atas com CPF e RG alcancaveis pelo retriever da Ana) e f5f7891 (mascaramento das 2 atas do Lume que o gate acusou; a causa-raiz - o mascarador e no-op no Windows por causa de CRLF - segue aberta para a sessao das atas).
