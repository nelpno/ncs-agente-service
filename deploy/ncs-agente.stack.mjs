...
export const ANA_IMAGE = "ghcr.io/nelpno/ncs-agente-service@sha256:20d6f4c1aa948d1a393f55b3d6abe4de8bb70fc5eec2d92edc2ff846112fe02a"; // 80127f3: manual do proprietario da SEIVA entregue INTEIRO como anexo (tool enviar_manual_proprietario + rota GET /manual/<token>, 30 min, mesmo mecanismo do CND). Pedido do Fernando 18/08 ("manda o arquivo inteiro, nao separa por trecho"), escopo fechado por ele as 23:37 ("So manual proprietario seiva"). Fica em data/documentos/, NAO em data/regimentos/: la o classificarDoc o rotularia como "Regimento Interno" e manual de construtora viraria regra de condominio dentro de uma multa. FALHA FECHADA por SENTINELA, nao por `hits.length === base.length`: com 1 item no catalogo esse teste nao distingue acerto de "devolveu tudo" e qualquer texto entregaria o manual da Seiva. Medido ao vivo 15/15 (garantia/manutencao na Seiva manda 6/6; cachorro e mudanca NAO mandam 6/6; Lume, que nao tem manual, NAO recebe o da Seiva 3/3). Suite 145/0. // (anterior 9da935c3) 51cd4e6: respostas do Fernando (WhatsApp 18/08). MUDANCA DA SEIVA = 72h ("72 horas") - ela nao estava em horarios-mudanca.json e a Ana respondia pelo regimento, onde a convencao diz 24h (4 de 4 formas de perguntar traziam esse trecho); cadastrada com o HORARIO da convencao (seg-sex 08h-17h, sab 08h-12h) e antecedencia vinda da regra geral. Medido com o modelo de producao: 6 de 6 respostas dizem 72h, 0 dizem 24h. `procedimento` vazio de proposito (rota interna nao confirmada). MULTA DO MARIO pelo R.I. ("Pode usar mais nova do regimento"): convencao_penalidades passa do Artigo 14 da convencao (decuplo) para o Artigo 8.o do Regimento de 2013, gradual (ADVERTENCIA -> ate 5 taxas -> dobro na reincidencia), verbatim; o que foi substituido fica em `substituiu` com motivo e data. verificar-catalogo APROVADO 12/12, fonte=regimento_literal. A minuta da Seiva foi APROVADA na AGI de 17/08 e vai a registro quinta 21/08 - segue SEM catalogo de multa ate a registrada sair. Suite 144/0. // (anterior 4c7000c7) 05302e2+3592776: SEIVA VILA HARMONIA entra no RAG e o Mario de Andrade ganha o Regimento Interno que faltava. Seiva: convencao do PDF digital (16 capitulos, 74 artigos, 116 paragrafos, 25 titulos do regulamento embutido), extraida com ORDEM VISUAL (fitz sort=True) - sem isso os titulos saiam no meio do paragrafo errado e "horario para fazer mudanca" respondia sobre cracha de trabalhador; agora a regra de mudanca chega no top-8 em 5 de 5 formas de perguntar. E MINUTA, nao a registrada: NAO tem catalogo em gerador/dados/ de proposito, entao carregarCondominio RECUSA e nao sai multa com base nela. Mario: convencao SUBSTITUIDA pela certidao do 1o RI - a .docx que estava no ar tinha o Artigo 20 (multa 2% + juros 1%) COLADO dentro do Artigo 19, entao a citacao de cobranca saia com o numero errado, e a alinea "l)" do Art. 10 e "i)" na registrada (o catalogo citava a errada). 4 texto_artigo corrigidos cirurgicamente (diff semantico: 4 de 97 campos), verificar-catalogo APROVADO 12/12. Paginas 1-8 da certidao (CPF/RG/endereco de dezenas de condominos) excluidas: os .md tem 0 CPF e 0 RG. Pino de nome sobre 205 nomes reais: 0 degradacoes. Suite 144/0. DRY_RUN_WRITES=true MANTIDO. // (anterior a481b165) c2d7171: o registro "CEDROS" das bases de mudanca/taxa/portaria e o VISTAS DO BOTANICO - CEDROS (resposta do Fernando, 10/08). Renomeado: "Cedros do Campo" (outro condominio, sem regra cadastrada) parava de casar e recebia horario de mudanca, itens da taxa e sistema de portaria do predio errado - era a unica colisao das 3 tools nos 55. Agora falha fechada. Limitacao no teste: "Cedros" sozinho ainda cai no Vistas em mudanca/portaria. // (anterior 827085b8 = 63a7eff: o card nao afirma mais que o contrato NAO VEIO quando ele veio. Conv 848: contrato enviado 13:14, lido pela Ana, cadastro criado 13:18 SEM conferencia (ela perguntou se havia mais paginas, ele respondeu outra coisa) -> laudo null -> "Nao veio contrato", e a equipe passou a tarde pedindo o que o cliente ja mandara. O rascunho grava `documento_recebido` (dossie com paginas na criacao) e o card separa "chegou mas nao foi conferido" de "nao veio". Rascunho antigo sem o campo mantem o texto conservador. Suite 141/0. // (anterior fbe7d6a3 = 5fba61f: nome de condominio nao localizado passa a FALHAR FECHADO. _filtrarCondos devolve a lista INTEIRA quando nenhum degrau casa, e o resolver_cadastro lia isso como "achei todos": em 10/08 (conversa 830) a moradora escreveu "Spazzio aboccato" (dois Z, um C a menos), a varredura rodou nos 59 e a Ana ofereceu 9 blocos do VALE SUPREMO; ao ela confirmar "Bloco 4", recebeu os canais da garantidora ASSISCON - de um predio que nunca citou. Agora: sem CPF/telefone sai antes da varredura com motivo condominio_nao_localizado (e economiza 59 chamadas); com CPF/telefone a varredura global continua (207 CPFs tem unidade em 2+ condominios), mas um match que venha so de unidade/nome e recusado. Teste test_condominio_nao_localizado.mjs escrito ANTES e visto falhando nos 5 asserts do bug. Junto: apostrofo passa a UNIR no match de nome (L'Harmonie x LHARMONIE) e o Estagiario ganha atas na tool description. Suite 140/0, gate 0 HARD/0 SOFT. DRY_RUN_WRITES=true MANTIDO. // (anterior a3b81ec1 = c07b0f6: CPF sai da copia que conversa.) A Ana devolvia 12 trechos com CPF LEGIVEL para MORADOR (7 documentos: Atlanta, Magnolias, Reserva do Campo, Salto Grande III, Studio Five, Tivoli) - medido pelo caminho exato dela (incluirAtas:false, 275 consultas, 1.647 trechos). Agora 0. Decisao do Fernando (WhatsApp 09/08, opcao 2): mascara so na copia que conversa; o texto que gera multa fica INTEGRO (o gerador le gerador/dados/*.json e nao passa pelo retriever, entao a base legal nao muda por construcao). O NOME do sindico permanece - regra diferente da ata de proposito. Mutacao: desligando a mascara, 11 trechos vazam e o teste fica vermelho. Suite 138/0. Leva junto b1b7c7e/7818cb5 (as 219 atas e a regra da data de CONVOCACAO): os ARQUIVOS entram na imagem da Ana, mas ela segue CEGA a eles - agent.mjs:222 passa incluirAtas:false DEPOIS do spread de args, entao o modelo nao consegue ligar, e o default do retriever e false. Provado no container apos o deploy. DRY_RUN_WRITES=true MANTIDO.
