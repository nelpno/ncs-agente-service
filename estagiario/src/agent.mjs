...
import * as CC from "../../src/codigo_civil.mjs"; // Pesquisa 2: fallback no Código Civil quando o RI/convenção não cobre
...
import * as CLUBE from "../../src/clube.mjs";    // REUSO: Clube NCS de Vantagens (empresas parceiras/descontos, GLOBAL) — dúvida de morador
import * as GAR from "../../src/garantidora.mjs"; // REUSO: base de garantidoras da Ana (condo com cobrança externa) — dúvida de morador
import * as COBP from "../../src/cobranca_parametros.mjs"; // REUSO: juros/multa/honorários por condomínio (planilha do Fernando, 06/08)
import * as SEK from "./secullum.mjs";           // NOVO: ponto/afastamento dos terceirizados (Secullum) — insumo do RH (LEITURA)
...
const TOOLS = [
  ...
  { type: "function", function: { name: "buscar_morador", description: "Busca o morador de uma unidade no Superlógica (nome + se é proprietário/inquilino). Use quando o usuário der o condomínio + número do apartamento, para preencher o destinatário SEM o usuário digitar o nome. Confirme o nome retornado antes de gerar. AJUDA, não autoriza: se voltar encontrado:false, NÃO insista na busca — siga com o que a equipe informou e omita o que ninguém souber. Se vier candidatos, mostre-os (unidade + responsável) e pergunte qual é.", parameters: { type: "object", properties: { condominio: { type: "string" }, unidade: { type: "string", description: "Número do apartamento, ex.: '132'." }, bloco: { type: "string", description: "Bloco/torre, se houver, ex.: '01'." } }, required: ["condominio", "unidade"] } } },
  { type: "function", function: { name: "dados_condominio", description: "Dados cadastrais do condomínio AO VIVO do Superlógica: nome, endereço, CEP, cidade/UF, CNPJ e o SÍNDICO atual (nome, cargo, e-mail — em associação o síndico é o 'Presidente'). Use quando a equipe perguntar o endereço, o CNPJ ou quem é o síndico de um condomínio. Nunca invente: se um campo vier null (ex.: sindico:null ou cnpj:null), diga que não consta no cadastro.", parameters: { type: "object", properties: { condominio: { type: "string", description: "Nome/slug do condomínio." } }, required: ["condominio"] } } },
  { type: "function", function: { name: "consultar_regimento", description: "Consulta os DOCUMENTOS do condomínio: Regimento Interno, Convenção **e as ATAS DE ASSEMBLEIA** (AGO/AGE — o que foi deliberado, aprovado ou reprovado; prestação de contas, previsão orçamentária, reajuste de taxa, obras, eleição de síndico). USE-A TAMBÉM para 'o que foi decidido na última assembleia', 'qual foi a última assembleia', 'ata da AGO', 'o que ficou aprovado' — você TEM as atas aqui (219 atas de 55 condomínios); nunca responda que não consegue consultar assembleia sem antes chamar esta ferramenta. Retorna trechos com a fonte (seção/artigo, ou 'ATA (DD/MM/AAAA)' quando o trecho vem de uma ata — a data no rótulo é a da CONVOCAÇÃO). Responda SEMPRE citando a fonte; se encontrou:false, não invente — ofereça encaminhar.", parameters: { type: "object", properties: { condominio: { type: "string" }, pergunta: { type: "string" } }, required: ["pergunta"] } } },
  { type: "function", function: { name: "consultar_codigo_civil", description: "FALLBACK JURÍDICO (Pesquisa 2): artigos do CÓDIGO CIVIL sobre condomínio (Lei 10.406/2002, arts. 1.314–1.358). Use SÓ quando a Convenção/Regimento Interno do condomínio NÃO cobrir a conduta (o consultar_regimento veio encontrou:false ou não trata do caso). Passe o TEMA do caso (ex.: 'infiltração/dano a outra unidade', 'barulho/perturbação do sossego', 'multa por reincidência/antissocial', 'obra que compromete a segurança', 'alteração de fachada', 'inadimplência/contribuição'). Retorna {encontrou, artigos:[{fonte, texto}]} — cite o artigo LITERAL com a fonte; se encontrou:false, não há base no CC → encaminhe. NUNCA invente artigo de lei.", parameters: { type: "object", properties: { tema: { type: "string", description: "O tema/conduta do caso, em palavras simples." } }, required: ["tema"] } } },
  { type: "function", function: { name: "gerar_documento", description: "Gera a notificação ou multa (uma MINUTA para o síndico assinar) — por padrão em WORD EDITÁVEL (.doc), para a equipe apagar os artigos que não se aplicam e complementar o relato antes de finalizar; gere em PDF só se pedirem a versão final. Só chame com TODOS os campos confirmados. O texto do artigo, a convenção e o cabeçalho são preenchidos pelo motor — você só fornece a classificação e o relato. Se o retorno vier motivo:'usar_codigo_civil', o regimento não cobre a conduta: mostre o artigo do Código Civil (codigo_civil.fundamento) à equipe e, se confirmarem, chame de novo com base_legal:'codigo_civil' (sem infracao_id). motivo:'sem_base_legal' = nem o regimento nem o Código Civil cobrem → encaminhe ao jurídico/síndico.", parameters: { type: "object", properties: {
      condominio: { type: "string" },
      tipo: { type: "string", enum: ["notificacao", "multa"] },
      base_legal: { type: "string", enum: ["catalogo", "codigo_civil"], description: "Fonte da base legal. Padrão 'catalogo' (infração do regimento do condomínio, via infracao_id). Use 'codigo_civil' SÓ depois de o sistema oferecer (motivo:'usar_codigo_civil') e a equipe confirmar: o documento sai como NOTIFICAÇÃO citando o artigo da lei; NÃO passe infracao_id nesse caso (o motor busca o artigo do Código Civil pelo relato)." },
      ...
      infracao_id: { anyOf: [{ type: "string" }, { type: "array", items: { type: "string" }, maxItems: 3 }], description: "Um id retornado por listar_infracoes — ou uma LISTA de ids quando pedirem mais de uma infração no MESMO documento (todas entram no enquadramento; não escolha uma só). Obrigatório para base_legal='catalogo' (o padrão); OMITA quando base_legal='codigo_civil'." },
    ...
    }, required: ["condominio", "tipo", "destinatario", "relato", "data_documento"] } } },
  ...
  { type: "function", function: { name: "gerar_resumo_financeiro", description: "RESUMO FINANCEIRO mensal (informativo enxuto de 1 página que a equipe envia aos gestores eleitos): saldo total em conta, RECEITA ordinária (Total de Receitas menos Fundo de Reserva, Rendimentos e Taxa Extra), DESPESA ordinária (Total de Despesas menos Investimento), resultado (receita − despesa) e situação (Positiva/Negativa), com texto informativo e rodapé LGPD. Use em 'resumo financeiro', 'informativo financeiro do mês', 'resumo do mês pros gestores/síndico'. NÃO confunda com gerar_relatorio_prestacao_contas (esse é o relatório completo por categoria, previsto x realizado, caixa e inadimplência). Informe o condomínio; mês/ano opcionais — mês omitido = último mês fechado.", parameters: { type: "object", properties: { condominio: { type: "string" }, mes: { type: "string", description: "Mês de referência: número 1-12 ou o nome (ex.: 'junho'). Opcional — se omitido, usa o último mês fechado." }, ano: { type: "integer", description: "Ano de referência (ex.: 2026). Opcional." }, formato: { type: "string", enum: ["pdf", "word"], description: "Formato de saída. Opcional — padrão 'pdf'. Use 'word' para documento editável." } }, required: ["condominio"] } } },
  ...
  { type: "function", function: { name: "consultar_clube", description: "Consulta as empresas parceiras e os descontos do Clube NCS de Vantagens. Lista GLOBAL, igual para TODOS os condomínios — NÃO precisa do condomínio. Use em 'quais empresas dão desconto', 'tem parceria com alguma farmácia/restaurante/pet', 'o Clube NCS tem X'. Passe um termo opcional (nome da empresa ou categoria); sem termo, devolve a lista completa (nome + categoria). Retorna { encontrou, empresas:[{nome, categoria, condicao, endereco, contato}] } (com termo) ou { encontrou, lista:[{nome, categoria}] } (sem termo). Repasse a condição/contato/endereço EXATAMENTE como vieram, sem resumir. encontrou=false → não invente empresa nem desconto.", parameters: { type: "object", properties: { termo: { type: "string", description: "Nome da empresa ou categoria (ex.: 'farmácia', 'Drogaven'). Opcional." } } } } },
  ...
  { type: "function", function: { name: "consultar_parametros_cobranca", description: "Condições de cobrança de inadimplência que o CONDOMÍNIO autorizou: se a NCS pode negociar, juros ao mês, multa, honorários, parcelamento máximo e prazo-limite. Chame sempre que a equipe perguntar quanto cobrar, se pode parcelar, ou quem conduz a cobrança de um condomínio NOMEADO. Retorna { encontrou, pode_cobrar, juros_mes, multa, honorarios_pct, parcelamento_max, janela_dias, encaminhar_para, resumo }. pode_cobrar:false = a NCS NÃO negocia nesse condomínio — passe o 'encaminhar_para'. Percentuais vêm como fração (0.01 = 1%); use o campo 'resumo', que já traz em %. NUNCA invente percentual, prazo ou parcelamento: se encontrou:false, diga que não consta e peça confirmação à diretoria.", parameters: { type: "object", properties: { condominio: { type: "string", description: "Nome do condomínio (ex.: 'Tivoli', 'Vancouver')." } }, required: ["condominio"] } } },
  ...
  // --- RH: ponto/afastamento dos terceirizados (Secullum). LEITURA. Anti-alucinação: só o que a API devolve. ---
  { type: "function", function: { name: "consultar_ponto", description: "Consulta o PONTO dos colaboradores/terceirizados no Secullum (dado do RH). Use para dúvidas do RH: AFASTAMENTOS (férias, atestados — 'quem está de férias/afastado', 'o fulano está de férias?'), PONTO/faltas ('o fulano bateu ponto essa semana?', 'batidas do fulano em maio') ou localizar um COLABORADOR pelo nome. Informe o assunto; funcionario (nome ou CPF) quando a pergunta é sobre uma pessoa; data_inicio/data_fim em ISO (YYYY-MM-DD) quando houver período (você sabe a data de hoje). Retorna { disponivel, encontrado, ... }: disponivel:false = integração ainda não configurada (avise que não está ligada); motivo:'ambiguo' = mostre as opcoes e pergunte qual (nome completo/matrícula); motivo:'nao_encontrado' = diga que não localizou, NÃO invente. Nunca invente nome, data ou afastamento — use só o que vier no retorno. No assunto=ponto, dias_sem_marcacao = dias sem batida registrada (pode ser folga, atestado ou dia ainda não fechado): apresente como 'sem registro', NÃO afirme que a pessoa faltou. Isto é dado de colaborador (RH), não de morador.", parameters: { type: "object", properties: {
      assunto: { type: "string", enum: ["afastamentos", "ponto", "funcionario"], description: "afastamentos = férias/atestados; ponto = batidas/faltas de um colaborador (exige funcionario); funcionario = só localizar/confirmar um colaborador." },
      funcionario: { type: "string", description: "Nome (completo ou parcial) ou CPF do colaborador. Obrigatório para assunto=ponto e assunto=funcionario; opcional em afastamentos (sem ele, lista todos do período)." },
      data_inicio: { type: "string", description: "Início do período em ISO YYYY-MM-DD (ex.: '2026-05-01'). Opcional — há um padrão sensato." },
      data_fim: { type: "string", description: "Fim do período em ISO YYYY-MM-DD. Opcional." },
    }, required: ["assunto"] } } },
];
...
async function runTool(name, args, ctx) {
  switch (name) {
    ...
    case "consultar_parametros_cobranca": return COBP.consultar_parametros_cobranca({ condominio: args.condominio });
    case "consultar_ponto": return await SEK.consultar_ponto(args, ctx.secullumDeps); // ctx.secullumDeps = seam de teste; undefined em prod → API real
    ...
    case "gerar_resumo_financeiro": {
      const out = await REL.gerar_resumo_financeiro(args);
      if (out.ok) ctx.lastDoc = { url: out.url, arquivo: out.arquivo, titulo: out.titulo };
      return out;
    }
